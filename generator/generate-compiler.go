...
func (classes *ClassCollection) generateCompiler(packageName string, license string) string {
	...
	code.Print("  return \"OpenAPIv2\"")
	code.Print("}")
	code.Print()

	classNames := classes.sortedClassNames()
	for _, className := range classNames {
		code.Print("func build%s(in interface{}) *pb.%s {", className, className)
			...
			if classFound {
				if propertyModel.Repeated {
					...
					if oneOfWrapper {
						...
						code.Print("if t != nil {")
						code.Print("x.Oneof = &pb.%s_%s{%s: t}", parentClassName, classModel.Name, classModel.Name)
						code.Print("}")
						code.Print("}")
					} else {
						code.Print("if helpers.MapHasKey(m, \"%s\") {", propertyName)
				...
				if isMap {
					...
					if mapTypeName == "string" {
						...
						code.Print("x.%s = make(map[string]*pb.%s, 0)", fieldName, mapTypeName)
					}
