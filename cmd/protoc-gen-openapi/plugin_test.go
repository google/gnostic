...
var openapiTests = []struct {
	...
	{name: "Service Tags", path: "examples/tests/servicetags/", protofile: "message.proto"},
}
...
func TestOpenAPIServiceTags(t *testing.T) {
	for _, tt := range openapiTests {
		fixture := path.Join(tt.path, "openapi_disable_service_tags.yaml")
		if _, err := os.Stat(fixture); errors.Is(err, os.ErrNotExist) {
			if !GENERATE_FIXTURES {
				continue
			}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := exec.Command("protoc",
				...
				"-I", "examples",
				path.Join(tt.path, tt.protofile),
				"--openapi_out=disable_service_tags=true:.").Run()
			if err != nil {
				t.Fatalf("protoc failed: %+v", err)
			}
			if GENERATE_FIXTURES {
