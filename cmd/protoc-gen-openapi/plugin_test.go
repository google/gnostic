...
var openapiTests = []struct {
	...
	{name: "OneOf Options", path: "example/tests/oneofoptions/", protofile: "message.proto"},
}
...
func TestOneOfOptions(t *testing.T) {
	for _, tt := range openapiTests {
		fixture := path.Join(tt.path, "openapi_oneofwrapped.yaml")
		if _, err := os.Stat(fixture); errors.Is(err, os.ErrNotExist) {
			if !GENERATE_FIXTURES {
				continue
			}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := exec.Command("protoc",
				...
				"-I", "examples",
				path.Join(tt.path, tt.protofile),
				"--openapi_out=oneof_type=wrapped:.").Run()
			if err != nil {
				t.Fatalf("protoc failed: %+v", err)
			}
			if GENERATE_FIXTURES {
