...
func main() {
	conf := generator.Configuration{
		...
		AutoMapping:      flags.Bool("auto_mapping", false, "parse methods without the google.api.http option."),
		ShortOperationID: flags.Bool("short_operation_id", false, `simplify the operation name. If "true", the the operation id will be the method name.`),
	}
